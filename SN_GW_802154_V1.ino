...
This is the current implementation. Part of the whole message list according specs is supported so far.
...
#define MQTT_CLIENT "HobbyIoT_GW"
...
const char* tout_msg = "TOUT";
    ...
    const int pin_LED_STA = 2; // GPIO2/D4
    const int pin_LED_ACT = 16; // GPIO16/D0 and also module internal LED
...
void setup() {
  ...
  Serial.println("MQTT-SN Gateway implementation V1");
  Serial.println("==================================");
...
void loop()
{
    ...
    if(message_received)
    {
      ...
      digitalWrite(pin_LED_ACT, LOW); // Blink the ACT LED
...
//            Serial.println();
        ...
        case(PUBLISH):
        {
          ...
          for(i=0;i<15;i++) pubMsg.data[i] = NULL;
...
//           }
      ...
      digitalWrite(pin_LED_ACT, HIGH); // End of ACT LED blink
   }
...
// -------------------------Routines for gateway operations -------------------------
    ...
    {
//      Serial.print("Check clients for timeout state at ");
...
//            mqttClient.publish((char *)clients[i].address,tout_msg);
            ...
            mqttClient.publish(conn_addr,tout_msg); // Send message to MQTT broker for the client timed out
...
//            while(clients[move_counter++].address != 0xFFFF); // find the end of the records
            ...
            clients[i].client_connected = clients[move_counter].client_connected;
            clients[i].timer = clients[move_counter].timer;
            clients[i].duration = clients[move_counter].duration; // all 4 fields
            
            clients[move_counter].address = 0xFFFF; // clear the free line
...
//      if(i>3) clientsn[0] = 'U';
      clientsn[0] = '0' + i;
//      clientsn[1] = 0x00;
//      String(i,DEC).toCharArray(clientsn,3);
...
//          mrf.address16_write(0x8266); // own address = 0x8266
//          mrf.rx_flush();
//          Serial.println("XXXXXXXX Clients connected 0 !!! Initializing MRF. XXXXXXXX");
//          clientsn[0]='X';
//      }

//      mqttClient.publish("clientsn",(char *)clientsn); // updates the number of connected nodes.
    }
